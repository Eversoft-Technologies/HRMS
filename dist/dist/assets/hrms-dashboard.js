/**
 * hrms-dashboard.js — Professional Real-Time Main System Dashboard (Flicker-Free)
 *
 * Provides:
 *  - Instant synchronous cached mount (eliminating startup flash and flickering)
 *  - Live KPI Cards with live database calculations and in-place DOM updates
 *  - Professional Department Breakdown (Doughnut / Pie / Bar View) with in-place chart sync
 ...
 *  - Isolated table search & filtering (no canvas re-renders or page tear-down)
 *  - Smooth in-place data updates on background refresh, focus, and visibility changes
 */
(function () {
  'use strict';

  var DASHBOARD_MOUNT_ID = 'hrms-live-dashboard-wrap';
  var CACHE_KEY = 'hrms_dash_stats_cache';
  ...
  var state = {
    ...
    lastUpdated: null,
    charts: {}, // map of id -> Chart instance
    searchQuery: '',
    deptFilter: 'all',
    statusFilter: 'all',
  ...
  function getCachedStats() {
    try {
      ...
      if (parsed && typeof parsed === 'object' && (parsed.totalEmployees !== undefined || parsed.attendance)) {
        return parsed;
      }
    } catch (_) {}
    return null;
  ...
  function injectDashboardStyles() {
    ...
    style.textContent = [
      ...
      '.hrms-dept-row-item { display: flex; align-items: center; justify-content: space-between; padding: 10px 8px; border-bottom: 1px solid var(--border2); cursor: pointer; border-radius: 6px; transition: all 0.15s ease; }',
  ...
  function fetchDashboardStats() {
    ...
    state.inFlightPromise = fetch('/api/dashboard/stats', { headers: headers })
      .then(function (r) {
      ...
      .catch(function (err) {
        console.warn('[HRMS Dashboard] Fetch warning:', err);
        state.loading = false;
        state.inFlightPromise = null;
      });
  ...
  function getFilteredEmployees(d) {
  ...
  function renderTableRowsHtml(filteredEmployees) {
    if (filteredEmployees.length === 0) {
      return '<tr><td colspan="8" style="text-align:center;padding:36px;color:var(--text3,#94a3b8);font-size:13px;">' +
    ...
    return filteredEmployees.map(function (emp) {
  ...
  function updateEmployeeTable() {
    var d = state.data;
    if (!d) return;
    var filtered = getFilteredEmployees(d);
    var total = (d.employees || []).length;

    var tbody = document.getElementById('dash-employee-tbody');
    if (tbody) {
      tbody.innerHTML = renderTableRowsHtml(filtered);
      var resetInEmpty = document.getElementById('hrms-dash-empty-reset');
      if (resetInEmpty) {
        resetInEmpty.addEventListener('click', function (e) {
          e.preventDefault();
          state.searchQuery = '';
          state.deptFilter = 'all';
          state.statusFilter = 'all';
          var sInput = document.getElementById('hrms-dash-search');
    ...
    var countEl = document.getElementById('dash-employee-count');
    if (countEl) {
      countEl.textContent = 'Showing ' + filtered.length + ' of ' + total + ' registered employees';
    }

    var hasActiveFilters = state.searchQuery || state.deptFilter !== 'all' || state.statusFilter !== 'all';
    var resetTopBtn = document.getElementById('hrms-dash-reset-filters');
    if (resetTopBtn) {
      resetTopBtn.style.display = hasActiveFilters ? 'inline-flex' : 'none';
    }

    var clearBtn = document.getElementById('hrms-dash-clear-search');
    if (clearBtn) {
    ...
    document.querySelectorAll('[data-status-filter]').forEach(function (btn) {
      var st = btn.getAttribute('data-status-filter');
      if (st === state.statusFilter) btn.classList.add('active');
      else btn.classList.remove('active');
    });

    document.querySelectorAll('[data-dept-filter]').forEach(function (btn) {
      var dp = btn.getAttribute('data-dept-filter');
      if (dp.toLowerCase() === state.deptFilter.toLowerCase()) btn.classList.add('active');
      else btn.classList.remove('active');
    });

    document.querySelectorAll('.hrms-dept-row-item').forEach(function (row) {
      var dp = row.getAttribute('data-dept');
      if (dp && dp.toLowerCase() === state.deptFilter.toLowerCase()) row.classList.add('active');
  ...
  function renderDeptListHtml(departments, totalDeptEmployees) {
    return departments.map(function (dp, idx) {
  ...
  function renderPendingActionsHtml(actions) {
    var list = actions || [
  ...
  function renderHighlightsHtml(highlights) {
    var list = (highlights && highlights.length) ? highlights : [
    ...
    return list.slice(0, 4).map(function (h) {
  ...
  function renderStatCard(idPrefix, label, val, sub, trend, color) {
    ...
    return '<div class="stat-card ' + esc(color) + '">' +
      ...
      '<div class="stat-value" id="' + idPrefix + '-val">' + esc(val) + '</div>' +
      '<div class="stat-sub" id="' + idPrefix + '-sub">' + esc(sub || '') + '</div>' +
      '<div class="' + trendClass + '" id="' + idPrefix + '-trend">' + esc(trend || '') + '</div>' +
    '</div>';
  }
  ...
  function renderFullDashboard(d) {
    ...
    var filteredEmployees = getFilteredEmployees(d);
    var hasActiveFilters = state.searchQuery || state.deptFilter !== 'all' || state.statusFilter !== 'all';

    var html = '<div id="' + DASHBOARD_MOUNT_ID + '">' +
      // Top Stat Cards
      '<div class="stat-grid" style="margin-bottom:24px;">' +
        renderStatCard('dash-kpi-emp', 'Total Employees', d.totalEmployees, d.activeEmployees + ' active in directory', '↑ +' + (d.newJoinersThisMonth || 0) + ' this month', 'blue') +
        renderStatCard('dash-kpi-pay', 'Monthly Payroll', d.monthlyPayroll, 'Estimated Gross', 'Live Calculation', 'purple') +
        renderStatCard('dash-kpi-pos', 'Open Positions', d.openPositions, d.totalOpenings + ' total openings', 'Active Job Posts', 'green') +
        renderStatCard('dash-kpi-att', 'Attendance Today', att.rate + '%', att.present + ' / ' + att.total + ' present', (att.wfh ? att.wfh + ' WFH • ' : '') + (att.late ? att.late + ' Late' : 'On track'), 'orange') +
      '</div>' +
      ...
      '<div class="grid-2 mb-16" style="gap:18px;margin-bottom:24px;">' +
        ...
        // Professional Department Breakdown Card
        '<div class="hrms-dash-pro-card" style="box-shadow: 0 4px 14px rgba(0,0,0,0.03);">' +
          '<div class="hrms-pro-card-header" style="align-items:flex-start;margin-bottom:8px;">' +
            '<div>' +
              ...
              '</div>' +
              '<div id="dash-dept-subtitle" style="font-size:11.5px;color:var(--text2);margin-top:3px;margin-left:11.5px;">' + departments.length + ' Departments • ' + totalDeptEmployees + ' Total Staff</div>' +
            '</div>' +
          ...
          '<div style="display:grid;grid-template-columns:1fr 1fr;align-items:center;gap:20px;min-height:200px;">' +
            ...
            '</div>' +
            '<div id="dash-dept-list" style="display:flex;flex-direction:column;justify-content:center;">' +
              renderDeptListHtml(departments, totalDeptEmployees) +
            '</div>' +
          '</div>' +
        '</div>' +
      '</div>' +

      // 3-Column Workforce, Pending Actions & Live Highlights Grid
      '<div class="hrms-dash-3col-row">' +
        ...
        '<div class="hrms-dash-pro-card">' +
          '<div class="hrms-pro-card-header">' +
            ...
            '<div style="display:flex;align-items:center;gap:6px;">' +
              '<span id="dash-att-rate-badge" style="font-size:11.5px;color:#10b981;font-weight:700;">' + att.rate + '% Attendance</span>' +
        ...
        '<div class="hrms-dash-pro-card">' +
          '<div class="hrms-pro-card-header">' +
            ...
            '</div>' +
          '</div>' +
          '<div id="dash-pending-actions-list" style="display:flex;flex-direction:column;">' +
            renderPendingActionsHtml(d.pendingActions) +
          '</div>' +
        '</div>' +
        ...
        '<div class="hrms-dash-pro-card">' +
          ...
          '</div>' +
          '<div id="dash-live-highlights-list" style="display:flex;flex-direction:column;">' +
      ...
      '<div class="card" style="margin-top:4px;box-shadow: 0 4px 14px rgba(0,0,0,0.03);">' +
        ...
        '<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:16px;flex-wrap:wrap;gap:10px;">' +
          '<div>' +
            ...
            '<div id="dash-employee-count" style="font-size:12px;color:var(--text3,#64748b);">Showing ' + filteredEmployees.length + ' of ' + (d.employees || []).length + ' registered employees</div>' +
          '</div>' +
          '<div style="display:flex;align-items:center;gap:10px;">' +
            '<button type="button" id="hrms-dash-reset-filters" class="btn-sm" style="font-size:11px;color:#ef4444;background:transparent;border:1px solid #fecaca;padding:5px 10px;border-radius:6px;cursor:pointer;' + (hasActiveFilters ? '' : 'display:none;') + '">✕ Reset Filters</button>' +
        ...
        '<div class="hrms-filter-container">' +
          '<div class="hrms-filter-row" style="margin-bottom:12px;">' +
            ...
            '<div class="hrms-search-box">' +
              ...
              '<button type="button" class="hrms-clear-search" id="hrms-dash-clear-search" title="Clear search" style="' + (state.searchQuery ? '' : 'display:none;') + '">✕</button>' +
            '</div>' +
          ...
          '<table style="width:100%;border-collapse:collapse;text-align:left;">' +
            '<thead>' +
              '<tr style="border-bottom:1px solid var(--border,#e2e8f0);background:var(--bg1,#f8fafc);">' +
                '<th class="hrms-table-header-sortable" data-sort="employeeId" data-label="EMP ID" style="padding:12px 10px;font-size:11px;color:var(--text2,#64748b);font-weight:700;letter-spacing:0.5px;">EMP ID ' + (state.sortColumn === 'employeeId' ? (state.sortAsc ? '▲' : '▼') : '') + '</th>' +
                '<th class="hrms-table-header-sortable" data-sort="name" data-label="EMPLOYEE" style="padding:12px 10px;font-size:11px;color:var(--text2,#64748b);font-weight:700;letter-spacing:0.5px;">EMPLOYEE ' + (state.sortColumn === 'name' ? (state.sortAsc ? '▲' : '▼') : '') + '</th>' +
                '<th class="hrms-table-header-sortable" data-sort="department" data-label="DEPARTMENT" style="padding:12px 10px;font-size:11px;color:var(--text2,#64748b);font-weight:700;letter-spacing:0.5px;">DEPARTMENT ' + (state.sortColumn === 'department' ? (state.sortAsc ? '▲' : '▼') : '') + '</th>' +
                '<th class="hrms-table-header-sortable" data-sort="role" data-label="DESIGNATION" style="padding:12px 10px;font-size:11px;color:var(--text2,#64748b);font-weight:700;letter-spacing:0.5px;">DESIGNATION ' + (state.sortColumn === 'role' ? (state.sortAsc ? '▲' : '▼') : '') + '</th>' +
                '<th class="hrms-table-header-sortable" data-sort="status" data-label="STATUS" style="padding:12px 10px;font-size:11px;color:var(--text2,#64748b);font-weight:700;letter-spacing:0.5px;">STATUS ' + (state.sortColumn === 'status' ? (state.sortAsc ? '▲' : '▼') : '') + '</th>' +
                '<th class="hrms-table-header-sortable" data-sort="location" data-label="LOCATION" style="padding:12px 10px;font-size:11px;color:var(--text2,#64748b);font-weight:700;letter-spacing:0.5px;">LOCATION ' + (state.sortColumn === 'location' ? (state.sortAsc ? '▲' : '▼') : '') + '</th>' +
                '<th class="hrms-table-header-sortable" data-sort="joinDate" data-label="JOIN DATE" style="padding:12px 10px;font-size:11px;color:var(--text2,#64748b);font-weight:700;letter-spacing:0.5px;">JOIN DATE ' + (state.sortColumn === 'joinDate' ? (state.sortAsc ? '▲' : '▼') : '') + '</th>' +
              ...
              '</tr>' +
            '</thead>' +
            '<tbody id="dash-employee-tbody">' +
              renderTableRowsHtml(filteredEmployees) +
            '</tbody>' +
          '</table>' +
        '</div>' +
      '</div>' +
  ...
  function updateLiveDashboard(d) {
    var wrap = document.getElementById(DASHBOARD_MOUNT_ID);
    if (!wrap || !document.contains(wrap)) {
      renderFullDashboard(d);
      return;
    }

    // 1. In-place update of KPI cards
    var att = d.attendance || { present: 0, total: 1, rate: 100, wfh: 0, late: 0, onLeave: 0 };
    ...
    setEl('dash-kpi-emp-val', d.totalEmployees || '0');
    setEl('dash-kpi-emp-sub', (d.activeEmployees || '0') + ' active in directory');
    setEl('dash-kpi-emp-trend', '↑ +' + (d.newJoinersThisMonth || 0) + ' this month');

    setEl('dash-kpi-pay-val', d.monthlyPayroll || '₹0');
    setEl('dash-kpi-pos-val', d.openPositions || '0');
    setEl('dash-kpi-pos-sub', (d.totalOpenings || '0') + ' total openings');

    setEl('dash-kpi-att-val', att.rate + '%');
    setEl('dash-kpi-att-sub', att.present + ' / ' + att.total + ' present');
    setEl('dash-kpi-att-trend', (att.wfh ? att.wfh + ' WFH • ' : '') + (att.late ? att.late + ' Late' : 'On track'));

    setEl('dash-att-rate-badge', att.rate + '% Attendance');

    // 2. Department Breakdown
    var departments = d.departments || [];
    var totalDeptEmployees = departments.reduce(function (s, x) { return s + x.count; }, 0) || d.totalEmployees || 1;
    setEl('dash-dept-subtitle', departments.length + ' Departments • ' + totalDeptEmployees + ' Total Staff');

    var deptListEl = document.getElementById('dash-dept-list');
    if (deptListEl) {
      deptListEl.innerHTML = renderDeptListHtml(departments, totalDeptEmployees);
      wireDeptListEvents();
    }

    // 3. Pending Actions & Highlights
    var pendingEl = document.getElementById('dash-pending-actions-list');
    if (pendingEl) pendingEl.innerHTML = renderPendingActionsHtml(d.pendingActions);

    var highEl = document.getElementById('dash-live-highlights-list');
    if (highEl) highEl.innerHTML = renderHighlightsHtml(d.highlights);

    // 4. Update Charts in-place without destroying canvases
    updateChartsInPlace(d);

    // 5. Update table rows
    updateEmployeeTable();
  }

  function updateChartsInPlace(d) {
    ...
    // Headcount Chart
    var chart1 = state.charts['headcount'];
    if (chart1 && d.headcountTrend && d.headcountTrend.length) {
      chart1.data.labels = d.headcountTrend.map(function (x) { return x.month; });
      chart1.data.datasets[0].data = d.headcountTrend.map(function (x) { return x.headcount; });
      if (chart1.data.datasets[1]) {
        chart1.data.datasets[1].data = d.headcountTrend.map(function (x) { return x.hires; });
      }
      chart1.update('none');
    } else if (document.getElementById('dash-chart-headcount')) {
      initHeadcountChart(d);
    }

    // Dept Chart
    var chart2 = state.charts['dept'];
    if (chart2 && d.departments && d.departments.length) {
      ...
      chart2.data.labels = deptLabels;
      chart2.data.datasets[0].data = deptCounts;
      chart2.update('none');
    } else if (document.getElementById('dash-chart-dept')) {
      initDeptChart(d);
    }

    // Attendance Chart
    var chart3 = state.charts['attendance'];
    if (chart3 && d.attendance) {
      var att = d.attendance;
      var presentOffice = Math.max(0, (att.present || 0) - (att.wfh || 0));
      chart3.data.datasets[0].data = [presentOffice, att.wfh || 0, att.late || 0, att.onLeave || 0];
      chart3.update('none');
    } else if (document.getElementById('dash-chart-attendance')) {
      initAttendanceChart(d);
    }
  }

  function wireDeptListEvents() {
    document.querySelectorAll('.hrms-dept-row-item').forEach(function (el) {
      el.addEventListener('click', function (e) {
        e.preventDefault();
        var targetDept = el.getAttribute('data-dept');
        if (targetDept) {
  ...
  function wireDashboardEvents() {
    ...
    // Reset filters
    var resetBtn = document.getElementById('hrms-dash-reset-filters');
    if (resetBtn) {
    ...
    document.querySelectorAll('[data-dept-filter]').forEach(function (el) {
      el.addEventListener('click', function (e) {
        e.preventDefault();
        var targetDept = el.getAttribute('data-dept-filter');
        if (targetDept) {
          state.deptFilter = (state.deptFilter.toLowerCase() === targetDept.toLowerCase()) ? 'all' : targetDept;
          updateEmployeeTable();
        }
    ...
    // Department chart switcher
    document.querySelectorAll('[data-chart-type]').forEach(function (el) {
      el.addEventListener('click', function (e) {
        e.preventDefault();
        var cType = el.getAttribute('data-chart-type');
        if (cType && state.deptChartType !== cType) {
          state.deptChartType = cType;
          document.querySelectorAll('[data-chart-type]').forEach(function (btn) {
            btn.classList.toggle('active', btn.getAttribute('data-chart-type') === cType);
            if (btn.getAttribute('data-chart-type') === cType && cType === 'doughnut') {
              btn.style.cssText = 'background:#f5f3ff;border-color:#8b5cf6;color:#6d28d9;font-weight:600;';
            } else {
              btn.style.cssText = '';
            }
          });
          initDeptChart(state.data);
        }
      });
    });
  ...
  function initHeadcountChart(d) {
    if (!window.Chart || !d || !d.headcountTrend || !d.headcountTrend.length) return;
    var ctxHeadcount = document.getElementById('dash-chart-headcount');
    if (!ctxHeadcount) return;

    if (state.charts['headcount']) {
      try { state.charts['headcount'].destroy(); } catch (_) {}
    }
    ...
    state.charts['headcount'] = new Chart(ctxHeadcount, {
      type: 'line',
      data: {
        labels: labels,
        datasets: [
  ...
  function initDeptChart(d) {
    if (!window.Chart || !d || !d.departments || !d.departments.length) return;
    var ctxDept = document.getElementById('dash-chart-dept');
    if (!ctxDept) return;

    if (state.charts['dept']) {
      try { state.charts['dept'].destroy(); } catch (_) {}
    }
    ...
    state.charts['dept'] = new Chart(ctxDept, chartConfig);
  }

  function initAttendanceChart(d) {
    if (!window.Chart || !d || !d.attendance) return;
    var ctxAtt = document.getElementById('dash-chart-attendance');
    if (!ctxAtt) return;

    if (state.charts['attendance']) {
      try { state.charts['attendance'].destroy(); } catch (_) {}
    }
    ...
    state.charts['attendance'] = new Chart(ctxAtt, {
      type: 'bar',
      data: {
  ...
  function tryMount() {
    ...
    if (isDashboard && container) {
      var wrap = container.querySelector('#' + DASHBOARD_MOUNT_ID);
      if (!wrap) {
        // If we have state data or cached data, mount immediately to eliminate blank screen jump
        if (state.data) {
          renderFullDashboard(state.data);
        } else {
        ...
        // Then fetch latest live data from server
        fetchDashboardStats();
  ...
  function boot() {
    ...
    // Theme observer: update charts smoothly with zero DOM tear-down
    ...
    // Live periodic background refresh (15s) with smooth in-place update
    ...
    // Instant sync on window focus / tab visibility
    ...
    // DOM Mutation observer (debounced with requestAnimationFrame)
    var scheduled = false;
